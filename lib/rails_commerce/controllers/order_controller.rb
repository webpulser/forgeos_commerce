module RailsCommerce
  ...
  #
  # ==== session's variables
  # * <tt>session[:order_shipping_method_detail_id]</tt> - an id of a <i>RailsCommerce::ShippingMethodDetail</i> choice by user
  # * <tt>session[:order_voucher_id]</tt> - an id of a <i>RailsCommerce::Voucher</i> using by user
  module OrderController
    ...
    def add_voucher
      voucher = RailsCommerce::Voucher.find_by_code(params[:voucher_code])
    ...
    def create_address
      ...
      if @address
        ...
        if current_user.addresses.size == 0
          [RailsCommerce::AddressDelivery, RailsCommerce::AddressInvoice].each do |type|
            current_user.addresses << type.create(params[:address])
          end
        else
          type = (params[:address][:type] == 'AddressDelivery') ? RailsCommerce::AddressDelivery : RailsCommerce::AddressInvoice
          current_user.addresses << type.create(params[:address])
        end
      end
      back_addresses
